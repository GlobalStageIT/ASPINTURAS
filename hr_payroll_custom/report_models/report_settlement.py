...
class ReportPayroll(models.AbstractModel):
    _name = "report.hr_payroll_custom.report_settlement_xlsx"
    ...
    def _generate_format(self, sheet, workbook ,record):
        ...
        sheet.merge_range('B3:F3','LIQUIDACIÓN DE CONTRATO DE TRABAJO', format)
        sheet.set_row(2, 30)
    ...
    def generate_xlsx_report(self, workbook, data, record):
        ...
        if record.company_id.primary_color and record.company_id.secondary_color:
            ...
            pr_color = "#00aaff"
            se_color = "#006aff"
        special_format = workbook.add_format({"font_color": "#FFFFFF","bg_color": pr_color,"align": "left", "border": True, "font_size": 11,"bold": True})
        ...
        for re in record:
            ...
            rh = self.env['hr.offboarding'].search([('payslip_id', '=', re.id)], limit=1)
            ...
            time_worked=days360(re.contract_id.date_start, rh.date_end_contract) + 1
            ...
            sheet.write(3, 2, rh.date_end_contract.strftime('%d/%m/%Y') or '')
            ...
            sheet.write(10, 2, rh.date_end_contract.strftime('%d/%m/%Y') or '',format_outer_border)
            ...
            cut_date = self.calcular_primas(re.contract_id.date_start,rh.date_end_contract)
            
            days_prima=days360(cut_date, rh.date_end_contract, cut_date) + 1
            sheet.write(17, 2, rh.date_end_contract.strftime('%d/%m/%Y') or '',format_outer_border)
            ...
            layoff_date = max(rh.date_end_contract + relativedelta(month=1, day=1), re.contract_id.date_start)
            
            sheet.write(17, 5, rh.date_end_contract.strftime('%d/%m/%Y') or '',format_outer_border)
            sheet.write(18, 5, layoff_date.strftime('%d/%m/%Y') or '',format_outer_border)
            sheet.write(19, 5, re.get_days_worked_year(rh.date_end_contract) or '',format_outer_border)
            ...
            sheet.write(22, 2, rh.date_end_contract.strftime('%d/%m/%Y') or '',format_outer_border)
            ...
            sheet.write(22, 5, rh.date_end_contract.strftime('%d/%m/%Y') or '',format_outer_border)
            sheet.write(23, 5, layoff_date.strftime('%d/%m/%Y') or '',format_outer_border)
            sheet.write(24, 5, re.get_days_worked_year(rh.date_end_contract) or '',format_outer_border)
            
            sheet.merge_range('B29:C29', "RESUMEN LIQUIDACIÓN PAGOS" or '',format)
            sheet.write(29,1, "CONCEPTO" or '',format2)
            sheet.write(29,2, "TOTAL" or '',format2)
            colum = 30
            for obj in table_header_devengados:
                sheet.write(colum, 1, obj or '',format_outer_border)
                colum+=1
            sheet.write(colum-1, 2, re.accrued_total_amount or '',format_number)
            
            colum2 = 30
            for obj in earn:
                sheet.write(colum2, 2, self.get_data(re.line_ids,obj) or '',format_number)
                colum2+=1
                
            sheet.merge_range('E29:F29', "RESUMEN DESCUENTOS LIQUIDACIÓN" or '',format)
            sheet.write(29,4, "CONCEPTO" or '',format2)
            sheet.write(29,5, "TOTAL" or '',format2)
            columt1 = 30
            for obj in table_header_deduccion:
                sheet.write(columt1, 4, obj or '',format_outer_border)
                columt1+=1
            sheet.write(columt1-1, 5, re.deductions_total_amount or '',format_number)
            
            columt2 = 30
            for obj in deduction:
                sheet.write(columt2, 5, self.get_data(re.line_ids,obj) or '',format_number)
                columt2+=1
            
            colum = colum if colum >= columt1 else columt1
            ...
            sheet.merge_range("C"+str(colum+4)+":"+"E"+str(colum+4), num2words(int(re.total_amount), lang='es').upper() or '')
