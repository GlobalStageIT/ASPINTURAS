...
class HrEmployeeBase(models.AbstractModel):
    ...
    def _get_consumed_leaves(self, leave_types, target_date=False, ignore_future=False):
        ...
        for allocation in allocations:
            ...
            vacation_taken = 0
            if allocation.holiday_status_id.is_vacation and allocation.employee_id and allocation.contract_id:
                vacation_taken = self._compute_taken(allocation.employee_id, allocation.contract_id)

            max_leaves += future_leaves
            allocation_data.update({
                'max_leaves': max_leaves - vacation_taken,
                'accrual_bonus': future_leaves,
                'virtual_remaining_leaves': max_leaves - vacation_taken,
                'remaining_leaves': max_leaves - vacation_taken,
                'leaves_taken': 0,
                    ...
                    if leave_type.requires_allocation == 'yes':
                        for allocation in sorted_leave_allocations:
                            ...
                            if leave.date_from != interval_start or leave.date_to != interval_end:
                                duration_info = employee._get_calendar_attendances(interval_start.replace(tzinfo=pytz.UTC),
    ...
    def _compute_taken(self, employee_id, contract_id):
        for record in self:
            ...
            return sum(novelty.quantity for novelty in novelty_records)
